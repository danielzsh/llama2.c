...
typedef struct {
  long malloc_run_state;
  long memory_map_weights;
  long build_tokenizer;
  long build_sampler;
  long encoding;
  long rmsnorm;
  long softmax;
...
void malloc_run_state(RunState *s, Config *p) {
  ...
  b.malloc_run_state = time_in_ms() - t;
}
...
void memory_map_weights(TransformerWeights *w, Config *p, float *ptr,
  ...
  b.memory_map_weights = time_in_ms() - t;
}
...
void build_tokenizer(Tokenizer *t, char *tokenizer_path, int vocab_size) {
  ...
  fclose(file);
  b.build_tokenizer = time_in_ms() - tt;
}
...
int main(int argc, char *argv[]) {
  ...
  printf("%ld time in generate loop\n", time_in_ms() - tt);
  b.total = time_in_ms() - t;
  printf("%ld milliseconds taken\n", b.total);
  printf("%ld matmul time\n", b.matmul);
  printf("%ld to memory map weights\n", b.memory_map_weights);
  printf("%ld to allocate run state\n", b.malloc_run_state);
  printf("%ld to build tokenizer\n", b.build_tokenizer);
  printf("%ld to encode\n", b.encoding);
  printf("%ld to decode\n", b.decoding);
